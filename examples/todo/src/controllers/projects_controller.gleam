...
pub fn show(
  ...
  case project_model.get(services.db, project_id) {
    Ok(project) -> show_with_todos(services, project, param.format)
...
fn show_with_todos(
  services: Services,
  project: types/project.Project,
  format: option.Option(String),
) -> Response {
  case todo_model.list_by_project(services.db, project.id) {
    Ok(tasks) -> {
      ...
      case format {
        ...
        _ ->
          html_response(
            status.ok,
            project_view.show_page(project, tasks, tags_by_todo),
          )
      }
    }
    Error(_) -> errors.internal_error()
